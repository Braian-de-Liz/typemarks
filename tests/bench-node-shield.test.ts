...
beforeAll(async () => {
  ...
  app = Fastify();
  app.setValidatorCompiler(({ schema, httpPart }: { schema: any; httpPart: string }) => {
    ...
    return (data: any) => {
      ...
      const mensagem = result.error instanceof ValidationError
        ? `Erro de validação: ${result.error.message}`
        : `Falha de validação no campo: ${httpPart}`;
      const error = new Error(mensagem);
      (error as any).statusCode = 400;
      return { error };
    };
  });
  app.post("/StronValid", { schema: { body: schemaBody } }, async (_req: any, reply: any) => {
...
describe("Benchmark Node Schema-Shield - Validação /StronValid", () => {
  test(
    ...
    async () => {
      ...
      console.log("  [Node Schema-Shield] Resultados do benchmark");
      console.log("-".repeat(50));
