...
app.post(
  ...
  validator("json", (data, c) => {
    if (!validate(data)) {
      return c.json({ success: false, errors: validate.errors }, 400);
    }
    return data;
  }),
...
describe("Benchmark Hono + AJV (Node) - Validação /StronValid", () => {
  test(
    ...
    async () => {
      ...
      console.log("  [Hono + AJV / Node] Resultados do benchmark");
      console.log("-".repeat(50));
